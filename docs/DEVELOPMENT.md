...
The editor has one scrolling body and a fixed total/action footer. Target width is 440 UI units, bounded by the viewport. Height is based on available space below its current position. Appearance at different resolutions and UI scales still needs verification.
...
Loading checks run under .NET, not inside Unity/Mono. They do not establish live multiplayer compatibility. Builds pass without compiler warnings; v0.5.2's layout has not been visually tested in game.
