...
Every announcement goes through the game's private `SingleGoodAllower.InvokeDisallowedGoodsChangedEvent(string)`, looked up by signature at startup. If a game update renames or changes it, `ModStarter` logs why at startup and allocations are frozen instead of half-applied: Apply and copies are refused with that reason, a mixed building stays mixed, and saved allocations keep limiting their buildings. The game still starts.
